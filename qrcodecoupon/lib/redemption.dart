...
class _RedemptionState extends State<Redemption> {
  ...
  void didChangeDependencies() {
    //didChangeDependencies means when the state of the widget changes (when the widget is built), it's different from initState which is only called once because it's called before the widget is built
  ...
  Future<void> redeemCoupon() async {
    ...
    if (doc.exists) {
      ...
      setState(() {
        ...
        couponId = couponId;
        validity = (doc.get('validity') as Timestamp)
            .toDate(); // to get the expiry date from firebase and convert it to DateTime format which is readable by flutter
        price = doc.get('price');
      });
    } else {
        ...
        children: [
          ...
          Container(
            child: ElevatedButton(
              ...
              style: ButtonStyle(
                backgroundColor:
                    MaterialStateProperty.all(const Color(0xFFC4E2A6)),
